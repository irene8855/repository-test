...
TOKENS = {             # отслеживаемые тикеры
    "BET", "FRAX", "EMT",
    "GMT", "SAND", "LDO", "SUSHI",
    "UNI", "APE", "AAVE", "LINK"
}

DEX_URLS = {           # ссылки на биржи
...
# ─── DexScreener → fallback GeckoTerminal ────────────
async def get_pools():
    async with aiohttp.ClientSession() as s:
        ...
        # ── fallback: GeckoTerminal /networks/polygon/pools ──
        ...
        if not pools:
            print("[WARN] GeckoTerminal вернул 0 пулов — пропускаем цикл.")
        return pools
